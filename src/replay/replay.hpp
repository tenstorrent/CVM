...
public:
  ...
  std::size_t check(const std::vector<logic_word>& observed,
                    std::uint64_t sim_time, const std::string& hier);
  ...
  void set_strict_x(bool strict) { strict_x_ = strict; }
...
private:
  ...
  void set_port(std::size_t bound, const std::string& state);
